...
def top(X, weights, impacts):
    ...
    if len(impacts) != np.shape(X)[1] or len(weights) != np.shape(X)[1]:
        print("length Parameters didn't match")
        sys.exit(0)
    ...
    result_df = pd.DataFrame(ans, columns=["Models", "Rank"])
    result_df["Models"] = result_df["Models"].astype(int)
